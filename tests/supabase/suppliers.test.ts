...
describe('suppliers gateway', () => {
  ...
  it('sbPushNcc là UPSERT-ONLY — không xoá NCC vắng mặt (chống wipe song song)', async () => {
    const c = await getViettoursClient();
    await sbPushNcc([
    ...
    // Đẩy danh sách CŨ (chỉ A) — B PHẢI còn nguyên; A vẫn cập nhật.
    await sbPushNcc([
    ...
    expect(list.map((x) => x.id).sort()).toEqual(['ncc-a', 'ncc-b']);
    expect(list.find((x) => x.id === 'ncc-a')!.name).toBe('A updated');
    expect(list.find((x) => x.id === 'ncc-a')!.sectors).toEqual(['transport']);
    // Xoá thật qua targeted sbDeleteNcc.
    await sbDeleteNcc('ncc-b', c);
    list = await once<Ncc[]>((cb) => sbSubscribeNcc(cb, c));
    expect(list.map((x) => x.id)).toEqual(['ncc-a']);
  });
