...
function tokenScore(token: string, hay: string): number {
  ...
  if (token.length >= 3 && isSubsequence(token, hay)) return 20;  // fuzzy nhẹ
  return 0;
}
...
export function matchesQuery(haystack: string, query: string): boolean {
  return searchScore(haystack, query) > 0;
}
 ...
 */
export function filterRank<T>(list: T[], query: string, getText: (item: T) => string): T[] {
  if (!query.trim()) return list;
  return list
    .map((item) => ({ item, score: searchScore(getText(item), query) }))
    .filter((x) => x.score > 0)
