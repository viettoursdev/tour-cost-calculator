...
describe('rateCardStore', () => {
  ...
  it('init subscribes to sbSubscribeMasterRC', () => {
    ...
    expect(sb.sbSubscribeMasterRC).toHaveBeenCalledTimes(1);
  });

  it('subscriber callback applies remote rates', () => {
    useRateCardStore.getState().init();
    const cb = vi.mocked(sb.sbSubscribeMasterRC).mock.calls[0][0];
    cb({
  ...
  it('setRates flips status to syncing and debounces supabase push by 2s', () => {
    ...
    expect(sb.sbPushMasterRC).not.toHaveBeenCalled();
    vi.advanceTimersByTime(2000);
    expect(sb.sbPushMasterRC).toHaveBeenCalledTimes(1);
    expect(vi.mocked(sb.sbPushMasterRC).mock.calls[0][1]).toBe('Tony (CEO)');
  });
  ...
  it('setRates uses "unknown" as pushedBy when no user is signed in', () => {
    ...
    vi.advanceTimersByTime(2000);
    const calls = vi.mocked(sb.sbPushMasterRC).mock.calls;
