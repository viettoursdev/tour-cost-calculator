...
describe('quoteHistoryStore', () => {
  ...
  it('init wires both subscribers', () => {
    ...
    expect(fb.fbSubscribeQuoteHistory).toHaveBeenCalledTimes(1);
    expect(fb.fbSubscribeDMCQuoteHistory).toHaveBeenCalledTimes(1);
  });

  it('regular subscriber updates quotes only; dmc subscriber updates dmcQuotes only', () => {
    useQuoteHistoryStore.getState().init(u);
    const cbReg = vi.mocked(fb.fbSubscribeQuoteHistory).mock.calls[0][0];
    const cbDmc = vi.mocked(fb.fbSubscribeDMCQuoteHistory).mock.calls[0][0];
    cbReg([entry({ id: 1 })]);
  ...
  it('init returns a function that unsubscribes both', () => {
    ...
    vi.mocked(fb.fbSubscribeQuoteHistory).mockReturnValueOnce(unsubReg);
    vi.mocked(fb.fbSubscribeDMCQuoteHistory).mockReturnValueOnce(unsubDmc);
