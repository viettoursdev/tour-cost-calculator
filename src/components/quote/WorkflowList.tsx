...
export function WorkflowList({ steps, users, onUpdate, onDelete, onReorder }: Props) {
  return (
    <Box>
      ...
      <SortableList onReorder={onReorder} handle=".wf-drag" deps={[steps.length]}>
        {steps.map((s, i) => (
          <Box key={s.id} sx={{ display: 'grid', gridTemplateColumns: COLS, gap: 1, alignItems: 'center', py: 0.25 }}>
            ...
            <TextField variant="standard" value={s.label} onChange={(e) => onUpdate(s.id, { label: e.target.value })} sx={inputSx} />
