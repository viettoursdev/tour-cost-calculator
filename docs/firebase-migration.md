...
**Post-cutover discovery — `v2` banned by Google.** The first verification mirror run (2026-06-17 03:11 UTC, `workflow_dispatch`) failed at the import step with `code: 16` UNAUTHENTICATED / `reason: ACCOUNT_STATE_INVALID` against the v2 SA `firebase-adminsdk-fbsvc@tour-cost-calculator-v2.iam.gserviceaccount.com`. Export from the new SRC (`4336c`) and artifact upload both succeeded — the issue is specifically that v2 is no longer writable. Diagnosis: Google has banned the `v2` project. As a temporary measure, the live-mirror step in `.github/workflows/backup.yml` is gated off via `if: false && success()` so hourly runs stay green and the artifact upload continues (the artifact path is the more reliable safety net anyway). **To resume the live mirror once v2 is unbanned:** delete the `false && ` prefix from the `if:` line on the `Import Firestore into DEST (mirror)` step, then trigger `Firestore Backup` once via `workflow_dispatch` to verify.

**Rollback:** revert the cutover steps above — swap `.env` / GitHub secrets back to `v2`, re-deploy rules to `v2` (already deployed at the previous swap), and flip `backup.yml` SRC/DEST. Rollback is currently **blocked** by the v2 ban; would require lifting it first.
