-- Khoá cứng cột phân loại `quotes.template` khi CẬP NHẬT một báo giá đã tồn tại.
--
-- Lý do: `template` là DUY NHẤT thứ phân tách "sheet Báo giá" (template <> 'dmc')
-- với "sheet DMC" (template = 'dmc'). Trước đây save_quote_state ghi đè
-- `template = excluded.template` ở nhánh ON CONFLICT, nên nếu draft trên client vô
-- tình mang template sai (ví dụ điều hướng thoát màn DMC bằng cách đặt tạm
-- template='intl' mà vẫn giữ currentQuoteId trỏ về báo giá DMC) thì lần Lưu kế tiếp
-- sẽ DỜI báo giá DMC sang sheet Báo giá (và ngược lại) — "lưu nhầm qua nhau".
--
-- Sửa: ở nhánh ON CONFLICT (cập nhật), GIỮ NGUYÊN template hiện có trong DB
-- (không lấy từ payload). Lần INSERT đầu tiên vẫn lấy template từ payload như cũ,
-- nên báo giá mới vẫn vào đúng sheet. Một báo giá KHÔNG bao giờ đổi sheet sau khi
-- đã tạo — đây là chốt chặn ở tầng dữ liệu, độc lập với mọi lỗi phía client.
--
-- Phần thân còn lại giữ y nguyên từ 0080 (Postgres không sửa từng phần thân hàm
-- được nên phải recreate trọn hàm). Khác biệt DUY NHẤT so với 0080: bỏ dòng
-- `template = excluded.template,` trong khối ON CONFLICT DO UPDATE.
...
begin
  ...
  on conflict (cloud_id) do update set
      -- template GIỮ NGUYÊN (không ghi đè) — chốt chặn chống dời sheet Báo giá↔DMC.
      pax = excluded.pax,
