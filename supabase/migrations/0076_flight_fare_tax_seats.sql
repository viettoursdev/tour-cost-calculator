-- Mở rộng hạng giá chuyến bay: tách Fare/Thuế (Total = Fare + Tax suy ra ở client)
-- và thêm block giữ chỗ: SL đặt cọc / khách xác nhận / được phép giảm.
-- Cột optional, KHÔNG đụng dữ liệu cũ (amount = Fare giữ nguyên).

alter table public.quote_flight_fares add column if not exists tax double precision;
alter table public.quote_flight_fares add column if not exists seats_deposit int;
alter table public.quote_flight_fares add column if not exists seats_confirmed int;
alter table public.quote_flight_fares add column if not exists seats_reducible int;

-- Cập nhật RPC save_quote_state để ghi các cột mới (chỉ thay đoạn insert fares;
-- phần còn lại giữ y nguyên từ 0021).
