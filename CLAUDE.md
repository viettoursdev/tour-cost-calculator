...
**Hourly artifact backup.** `.github/workflows/backup.yml` runs every UTC hour, exports the production Firestore via `scripts/firestore-export.mjs`, and uploads `firestore-dump.json` as a workflow artifact with 30-day retention. The single secret `FIREBASE_BACKUP_SRC_SA_JSON` holds the production project's service-account JSON. There is no second Firestore destination — recovery is a manual download from the Actions run page followed by `SA_PATH=<key.json> node scripts/firestore-import.mjs` against any target project. Worst-case data-loss window: ~1 hour for committed writes, plus the time it takes to identify a failure and restore. Cost: artifact storage + ~720 export-only Firestore reads/month, negligible.
