...
**AI features via Cloudflare Worker.** `src/lib/aiWorker.ts:callAIWorker(path, body)` proxies to `cloudflare-worker/viettours-ai-worker.js` (holds `ANTHROPIC_API_KEY` + R2). Endpoints: `/ai` (tour program, Haiku), `/ocr` (structure-preserving Markdown, Sonnet), `/translate` (visa-grade VI→EN, Sonnet), `/chat` (assistant tool-use, Sonnet, optional `web_search`). **The worker IS auto-deployed by CI** — `.github/workflows/worker-deploy.yml` runs `wrangler deploy` on push touching `cloudflare-worker/**` or `wrangler.toml` (sets `SUPABASE_PROJECT_REF` via `--var`, `--keep-vars` to preserve dashboard vars; cron from `wrangler.toml [triggers]`). It also has a `scheduled` handler for the morning digest (`runMorningDigest`).
