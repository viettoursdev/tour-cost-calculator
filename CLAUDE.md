...
| `hr_attendance` | **Chấm công / bảng công** theo NGÀY bằng mã (1 dòng = NV × tháng `period`, `days` jsonb {ngày→{code,note}}, `summary` tính sẵn, `status` draft/published/locked, `confirmation`+`feedback`). legacy_id tất định `att-{empLegacyId}-{period}` → import lại không trùng. Mã công ở `src/lib/attendance/attendanceCodes.ts`; tính công thuần `attendanceCalc.ts`. Tab "Chấm công" trong HRView (Gantt sửa ô trực tiếp, gate `manageHR`; xem scope phòng qua `visibleEmployees`). NV thường tự xem/xác nhận/báo sai sót qua thẻ Home "Bảng công của tôi" → `AttendanceSelfDialog` (khớp user↔NV bằng `myEmployee`). Import Excel `importAttendanceExcel.ts` + xuất `exportAttendanceExcel.ts`. Gateway `sb*HrAttendance`, store `attendanceStore`. RLS: `is_viettours_user`; scope/duyệt ở tầng app như các bảng hr_*. |
